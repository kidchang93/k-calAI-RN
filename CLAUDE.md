...
| 음식 이미지 분류 | `POST` | `/api/predict` | 동일 | 일치 (Gemini 비전, 한국어 라벨). **2026-07-16: 응답이 `predictions`(한 음식의 후보 나열) → `foods`(사진 속 **서로 다른 음식들**, 각 `label`·`score`·`portion_g`, 최대 10)로 바뀜 — 서버 22장.** 쿼터는 **사진당 1건**(음식 개수 무관). 앱(`calorie-api.ts`)은 배포 전환기 대비로 `foods` 없으면 `predictions`를 foods로 받아들인다. 끼니 구성 화면이 각 food를 초안 항목으로 만들고 음식마다 `/api/nutrition/estimate`(쿼터 0)로 kcal을 채운다. 응답의 **`vision_used`·`vision_limit`**(오늘 사용량, 사진당)로 남은 건수를 표시. (레거시 `/api/gpt-predict`는 2026-07-12 제거) |
