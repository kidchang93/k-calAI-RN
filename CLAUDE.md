...
| **운동 기록** | — | `GET /api/exercise-types`, `POST·GET /api/exercises`, `PUT·DELETE /api/exercises/{id}`, `GET /api/me/exercise-summary` | 동일 | 일치 (2026-07-21, `exercise-api.ts`, 전부 Bearer. 서버 `docs/DATA_MODEL.md` 25장·`docs/ACTIVITY_GUIDANCE.md` 3-2). **플랫폼 중립 API** — 앱·웹이 같은 레벨이고, 나중의 기기 연동은 `source`가 느는 입력 경로일 뿐이다. 화면은 `app/exercises/index.tsx`(주간 달성률 + 추가 + 오늘 목록). kcal 은 **서버가 MET×체중×시간으로 산출**하므로 앱에서 계산하지 않는다(체중 미등록이면 null). 운동 종류 목록·표시명·고지 문구도 서버가 준다 |
