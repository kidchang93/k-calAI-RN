...
| 음식 이미지 분류 | `POST` | `/api/predict` | 동일 | 일치 (Gemini 비전, 한국어 라벨). 칼로리·영양은 `/api/nutrition/estimate`. 응답에 **`vision_used`·`vision_limit`**(오늘 사용량)이 포함된다 — 기록 탭이 별도 조회 없이 남은 건수를 표시. (레거시 `/api/gpt-predict`는 2026-07-12 서버·앱 모두 제거) |
