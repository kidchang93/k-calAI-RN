...
export async function startCheckout(planCode: string): Promise<BillingCheckout> {
  ...
  const data = await readOk(response, '결제 준비 실패', BILLING_ERRORS);

  return ensure(isBillingCheckout(data) ? data : null);
}
...
export async function confirmBilling(input: {
  ...
  return ensure(parseMySubscription(await readOk(response, '결제 확인 실패', BILLING_ERRORS)));
}
