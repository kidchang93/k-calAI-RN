...
- 색상은 소문자 6자리 hex (`#ffffff`, `#60beb8`). `docs/DESIGN.md`의 팔레트를 벗어나지 않습니다.
- **면과 글자는 같은 색을 쓰지 않습니다.** 배경·게이지·버튼은 기준 민트/코랄(`#60beb8`·`#ea8989`),
  그 위의 글자·아이콘은 딥 톤(`#2a7d76`·`#b8524e`) 또는 잉크(`#22211f`)입니다 (`docs/DESIGN.md` 디자인 토큰).
...
### 판정으로 읽히는 표현을 쓰지 않는다 (2026-09-13, KCAL-16)

이 앱은 판단을 대행하지 않고 **재기만 한다**(서버 `docs/PRODUCT_STRATEGY.md` §0-1). "초과"라고 쓰는 순간 넘으면 안 되는 선을 앱이 그은 것이 된다. 수치·기간 비교 문구에는 아래 표현을 쓰지 않습니다.

| 쓰지 않는다 | 대신 |
|---|---|
| 초과 | `기준보다 많아요` · `목표보다 많은` · 범례처럼 짧은 자리는 `기준 위`·`목표 위` |
| 미달 | `기준보다 적어요` · `목표 아래` |
| 달성 | `목표 이내` · `목표만큼 했어요` |
| 위반 | 쓰지 않는다 |

- **색은 지우지 않습니다.** 코랄은 눈에 띄라고 쓰는 색이지 잘못했다는 뜻이 아닙니다.
- **그림도 같은 말을 해야 합니다.** 문구를 바꾸고 기준 이내에 체크(`check-circle`)를 달면 여전히 합격 도장입니다 — 중립 아이콘(`info-outline`)을 씁니다.
- 기준선을 보여줄 때는 **누가 그은 선인지**(서버가 주는 `basis`)를 함께 적습니다. 특히 리포트는 진료실에서 의료진이 보는 종이입니다.
- 에러·요금제 안내(`요금제 한도를 초과했습니다` 등)는 건강 수치 판정이 아니라 대상이 아닙니다.
