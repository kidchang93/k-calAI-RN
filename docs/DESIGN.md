...
- **식단 기록을 저장하지 않습니다.** 분석 결과는 화면 상태로만 존재합니다.

## 미완성 설계 (구현 전 반드시 확인)

| 항목 | 현재 상태 | 필요한 결정 |
|------|-----------|-------------|
| 세션 영속화 | 메모리에만 존재. 재시작 시 소실 | `expo-secure-store` 도입 여부 |
| 토큰 사용 | `access_token`을 보관만 하고 헤더에 넣지 않음 | 어떤 요청에 `Authorization`을 붙일지 (서버에 검증 코드가 없음) |
