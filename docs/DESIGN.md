...
| 추천 403은 `ConsentRequiredError`로 동의 화면 리다이렉트 | `services/recommendation-api.ts` | 질병·알러지를 쓰는 조회라 온보딩 화면들의 403 규약과 동일하게 처리. **2026-09-13(KCAL-22): 온보딩 밖 화면(추천·`me/conditions`·`me/allergies`)은 `/onboarding/consent`가 아니라 `/me/consents`로 보낸다** — 이전 버전 동의자는 403 을 받지만 데이터가 남아 있는데, 온보딩 동의 뒤에는 프리필 없는 전체 교체 PUT 화면(질병·알러지·신체·목표)이 이어져 기존 값을 덮어쓴다. 온보딩 안의 403(`onboarding/conditions`·`allergies`)만 온보딩 동의로 되돌린다 |
...
| 반려동물은 **앱에서 제거**, 그룹 챌린지는 **숨김** (2026-08-18 / 2026-07-25·2026-09-13) | `app/pets/` 삭제, `app/groups/[id].tsx`(챌린지 미표시), `app/groups/create.tsx`(챌린지 종류 숨김) | 목표(서버 `PRODUCT_STRATEGY.md` §0-1 — 식이요법이 필요한 사람의 판단 근거)와 연결점이 없고 사용 0건이었다. 2026-07-25에는 진입점만 숨겼으나 **요금제·가입 화면에는 "반려동물 N마리"가 판매 스펙으로 남아 있었다** — 숨김은 돈 받는 화면까지 닿지 않는다는 것이 재발로 확인됐다. 그래서 앱에서는 화면·폼·API 클라이언트를 지웠다. **서버는 그대로 둔다**: `users` FK 연쇄와 `account_service.delete_account`, `tests/test_account_service.py`의 handled 집합까지 정리해야 하고 되돌리기 어렵다. 등록된 반려동물은 0마리라 데이터 손실도 없다. **챌린지는 지우지 않고 숨긴다**(2026-09-13 결정 — 안 쓰는 기능은 삭제가 아니라 숨김으로 두고 필요해지면 구현한다, KCAL-14). 07-25에 화면만 숨겼는데 그룹 만들기에 '챌린지' 종류가 계속 떠 있어 그것도 숨겼다. 컴포넌트·클라이언트·서버 API는 그대로다. 순위·경쟁은 만성질환 기록을 성적표로 만들 수 있어 되살릴 때 KCAL-18의 경계를 먼저 본다 |
...
| **진료 탭은 세 묶음으로 나눈다 — 진료 준비 / 식단과 검사 수치 / 몸과 활동** (2026-09-15) | `app/(tabs)/trends.tsx`의 `SectionLabel` | 성격이 다른 카드 여덟 개가 제목 모양도 제각각인 채 한 줄로 이어져 흩어져 보였다. 탭 정의(서버 `CARE_LOOP.md` §7 "진료를 준비하고 결과를 받아 적는 곳")대로 가르되 **카드는 옮기지도 빼지도 않고 묶음 제목만 붙였다** — 식단 바로 아래 검사 수치(근거 쌍), 권장 활동량 바로 아래 조언(기준 쌍) 결정은 그대로다. 그래프·캘린더 토글은 식단 묶음에만 영향을 주므로 페이지 제목 줄에서 그 묶음 제목 줄로 옮겼다. 묶음 제목은 안쪽 소제목(잉크 17pt)보다 한 단계 위(강조색 19pt + 위쪽 구분선) |
...
| 낡은 민감정보 동의는 **'동의함'으로 그리지 않고 다시 동의를 받는다** (2026-09-13, KCAL-22) | `app/me/consents.tsx`, `constants/consent.ts`, `components/consent-notice.tsx` | v1.0 문구는 "식단 추천에서 거르는 데만"이라 했는데 실제로는 검사 수치·진료 메모도 같은 동의로 받고 경고·주간 조언·리포트에도 썼다(개인정보 보호법 제23조 — 알린 범위에서만 처리). 서버가 v1.0 동의를 무효로 보고 403 을 주므로, 체크 표시를 그대로 두면 화면이 서버와 다른 말을 한다. `is_current === false`면 무엇이 바뀌었는지 한 줄과 '다시 동의하기'를 그리고, 철회는 계속 열어 둔다. 동의 관리에도 동의 버튼이 있으므로 **온보딩과 같은 고지 전체**(항목·목적·보유 기간·제3자 제공·거부 시 불이익)를 그린다 — 두 화면이 문구를 따로 가지면 한쪽만 고쳐져, 한 사람이 어느 화면에서 동의했는지에 따라 다른 것에 동의하게 된다. 그래서 문구를 버전 상수와 같은 파일에 뒀다. 거부 시 불이익은 **실제로 막히는 것만** 적는다 — 예전 "식단 추천은 일반 가이드로 제공"은 추천이 403 이라 사실이 아니었다 |
...
| 네트워크·서버 오류 | 화면 내 `errorBox` 배너 (`#fbeaea` 배경 + `error-outline` 아이콘) |
...
**2026-08-14: 팔레트 전면 교체 — 블루 계열 → 민트 + 웜 뉴트럴 + 코랄.** 메인은 민트(`#60beb8`)이고,
반대 동작(삭제·해지·초과·오류)에만 코랄(`#ea8989`·`#ed9c89`)을 씁니다. 뉴트럴은 차가운 회색이 아니라
웜 그레이라 민트와 같은 온도로 붙습니다.

### 기준 색 (지급받은 11색)

| 계열 | 값 |
|------|-----|
| 뉴트럴 | `#22211f` `#5c5b57` `#a9a6a1` `#e4e2de` `#f7f6f4` |
| 민트 | `#60beb8` `#75c3bb` `#99d2ce` `#bee2dd` |
| 코랄 (반대 동작) | `#ed9c89` `#ea8989` |

### 파생 톤 (텍스트 대비용, 위 색의 명도 변형)

밝은 민트·코랄을 흰 배경 위 **텍스트/아이콘**에 쓰면 대비가 2.2:1까지 떨어져 읽히지 않습니다.
면(배경·게이지·버튼)은 기준 색을, **글자와 아이콘은 아래 딥 톤**을 씁니다 (전부 흰 배경 대비 4.5:1 이상).

| 용도 | 값 | 흰 배경 대비 |
|------|-----|--------------|
| 강조 텍스트·아이콘·링크 | `#2a7d76` | 4.9:1 |
| 위험·오류 텍스트 | `#b8524e` | 4.8:1 |
| 주의(보통 등급) 텍스트 | `#a4603f` | 4.9:1 |
| 옅은 강조 배경 | `#eef7f5` | — |
| 오류·위험 배경 | `#fbeaea` | — |
| 주의 배경 | `#fbeee7` | — |
...
| 강조 / 프라이머리 (면) | `#60beb8` |
| 강조 (텍스트·아이콘) | `#2a7d76` |
| 강조 배경 | `#bee2dd`(진한 틴트·배지), `#eef7f5`(옅은 정보 박스) |
| 제목 텍스트 | `#22211f` |
| 본문·보조 텍스트 | `#5c5b57` |
| 흐린 텍스트·placeholder | `#a9a6a1` |
| 입력/채움 배경·보더 | `#e4e2de` |
| 비활성 (프라이머리 버튼) | `#99d2ce` |
| 성공 (면 / 텍스트) | `#60beb8` / `#2a7d76` |
| 오류 (텍스트 / 면 / 배경) | `#b8524e` / `#ea8989` / `#fbeaea` |
| 카카오 브랜드 (버튼 배경 전용) | `#fee500` + 텍스트 `#22211f` |
| 라운드 | `8` (pill은 `999`) |
| 눌림 상태 | `opacity: 0.74` |

### 영양 등급 3단계 (`components/nutrient-chips.tsx`, `day-nutrients-card.tsx`)

| 등급 | 텍스트 | 배경 / 게이지 |
|------|--------|---------------|
| low (낮음·정상) | `#2a7d76` | `#bee2dd` / `#60beb8` |
| mid (보통·근접) | `#a4603f` | `#fbeee7` / `#ed9c89` |
| high (높음·초과) | `#b8524e` | `#fbeaea` / `#ea8989` |

### 민트 면 위의 글자는 흰색이 아니라 잉크입니다

`#60beb8` 위 흰 글자는 대비 **2.2:1**이라 야외에서 사라집니다. 만성질환군이 주 사용자라
CTA가 흐린 것은 그 자체로 결함입니다. 그래서 민트·코랄 버튼의 라벨·아이콘·스피너는 전부
`#22211f`입니다 (민트 위 7.3:1, 코랄 위 6.4:1). 흰 글자는 **잉크 배경 위**(`app/auth.tsx`의 `logoMark`)에만 남깁니다.
