...
| **D. EAS Build (Android APK)** ⭐ | **카카오 로그인이 실제로 된다** | 전부 (실제 앱과 동일) | 빌드 10~20분, 로컬 설치 0 |

**대부분의 확인은 A로 끝난다.** 이 저장소의 기능 검증(추천·기록·운동·챌린지·코칭)은 전부 웹에서 이뤄졌다.
실기기가 꼭 필요한 것은 카메라 촬영, 딥링크 복귀, 네이티브 화면 크기·안전영역 정도다.

**네이티브 카카오 로그인을 처음 검증하려면 D가 가장 빠르다** (2026-08-03 추가, §D).

---

## D. EAS Build — Android APK (네이티브 검증의 최단 경로)

`eas.json`의 **`preview`** 프로파일이 이 용도다. 클라우드에서 빌드하므로 **로컬 툴체인이 필요 없다**
(Android SDK·CocoaPods 불필요). Apple 개발자 계정도 필요 없다 — Android는 무료다.

```bash
cd k-calAI-RN
npx eas login                                        # Expo 계정 (무료)
npx eas build --platform android --profile preview
```

빌드가 끝나면 나오는 **링크를 폰에서 직접 열어** APK를 받는다. **USB 케이블 불필요.**
(안드로이드가 '출처를 알 수 없는 앱' 설치를 물으면 허용한다.)

### 왜 이 경로에서 카카오 로그인이 되나

`preview` 프로파일은 `EXPO_PUBLIC_API_ORIGIN=https://api.kcalai.link`를 박아 **운영 서버**를 본다.
카카오 허용 IP에 등록된 것이 그 서버라, 위 B의 IP 제한 문제(알려진 문제 12)를 우회한다.
딥링크도 맞는다 — 앱 `scheme`(`app.json`)과 서버 `APP_DEEPLINK_SCHEME`이 둘 다 `kcalairn`이고,
`services/auth-api.ts:123`이 네이티브에서 `platform=native`로 열어 `kcalairn://auth`로 되돌아온다.

> ⚠️ **이 빌드가 만드는 기록·결제는 운영 데이터다.** 로그인·화면 확인용으로만 쓰고, 남긴 기록은 지운다.

### 무엇을 확인하는가 (첫 실행 체크리스트)

| # | 확인 | 실패하면 |
|:-:|---|---|
| 1 | 앱이 뜨는가 (스플래시 → 로그인 화면) | 빌드/네이티브 설정 문제 |
| 2 | **카카오 로그인 → 앱으로 되돌아오는가** | 딥링크 스킴 불일치 — 여기가 한 번도 검증 안 된 지점이다 |
| 3 | 온보딩 → 홈 진입 | 세션이 `expo-secure-store`에 저장되는가 |
| 4 | 카메라로 음식 촬영 → 인식 | 권한 문구(`app.json` expo-image-picker) 동작 |
| 5 | 앱을 껐다 켜도 로그인 유지 | 세션 영속화 |

### 다른 프로파일

| 프로파일 | 용도 | 비고 |
|---|---|---|
| `preview` | **독립 실행 APK** — Metro 없이 폰만으로 실행 | 첫 검증은 이것 |
| `development` | dev client — Metro에 붙어 핫리로드 | **`expo-dev-client` 설치 필요** (`npx expo install expo-dev-client`) |
| `production` | 스토어 제출용 AAB | IAP 착수 후 |
