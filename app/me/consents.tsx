...
export default function ConsentsScreen() {
  ...
  // 같은 규칙) — confirmDialog로 두 플랫폼에서 같게 확인받는다 (services/dialog.ts).
  const requestRevoke = async () => {
    // **파기 범위가 바뀌면 이 문구도 함께 바꾼다** (2026-08-19). 서버가 지우는 목록은
    // `services/consent_service._destroy_sensitive_data`(건강 프로필·질병·알러지·검사 수치 행
    // 삭제, 진료 메모만 비움)이고, 사용자가 무엇을 잃는지 모르고 누르면 고지의 의미가 없다.
    const confirmed = await confirmDialog({
      title: '동의를 철회할까요?',
      message:
        '입력한 혈액형·질병(병기 포함)·알러지와 기록해 둔 검사 수치, 진료에서 들은 메모가 즉시 삭제되고 되돌릴 수 없어요. 다시 동의해도 복구되지 않습니다. 저장돼 있던 식단 추천 목록도 함께 지워져요. 식단 추천과 주간 조언을 쓸 수 없고, 기록할 때 알러지·질환 경고도 뜨지 않아요. 사진 기록과 칼로리 계산, 진료 일정은 그대로 쓸 수 있어요.',
    ...
    if (confirmed) {
      await submit(() => revokeConsent('sensitive_health'));
    }
  };
  ...
  return (
    <Screen>
      ...
      {isLoading ? (
        <LoadingState label="동의 내역을 불러오는 중입니다." />
      ) : (
        <>
  ...
  return (
    <Pressable
      ...
      <Text style={styles.requiredMeta}>
        {consent === null ? '기록 없음' : `${formatIsoYearMonthDay(consent.agreed_at)} 동의`}
      </Text>
...
function describeHealthStatus(consent: ConsentRecord | null, state: HealthConsentState): string {
  ...
  if (consent.revoked_at !== null) {
    return `${formatIsoYearMonthDay(consent.revoked_at)}에 철회함`;
  }
